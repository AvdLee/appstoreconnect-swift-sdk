...
public struct APIConfiguration {
    ...
    /// Whether the provided API credentials are for an enterprise account.
    ...
    ///   - isEnterprise: Whether the provided API credentials are for an enterprise account. Defaults to false.
    public init(issuerID: String, privateKeyID: String, privateKey: String, expirationDuration: TimeInterval = 60 * 20, isEnterprise: Bool = false) throws {
    ...
    ///   - expirationDuration: The token's expiration duration in seconds. Tokens that expire more than 20 minutes in the future are not valid, so set it to a max of 20 minutes. Defaults to 20 minutes.
    ///   - isEnterprise: Whether the provided API credentials are for an enterprise account. Defaults to false.
    public init(individualPrivateKeyID: String, individualPrivateKey: String, expirationDuration: TimeInterval = 60 * 20, isEnterprise: Bool = false) throws {
    ...
    ///   - expirationDuration: The token's expiration duration in seconds. Tokens that expire more than 20 minutes in the future are not valid, so set it to a max of 20 minutes. Defaults to 20 minutes.
    ///   - isEnterprise: Whether the provided API credentials are for an enterprise account. Defaults to false.
    public init(issuerID: String, privateKeyID: String? = nil, privateKeyURL: URL, expirationDuration: TimeInterval = 60 * 20, isEnterprise: Bool = false) throws {
        self.issuerID = issuerID
    ...
    ///   - expirationDuration: The token's expiration duration in seconds. Tokens that expire more than 20 minutes in the future are not valid, so set it to a max of 20 minutes. Defaults to 20 minutes.
    ///   - isEnterprise: Whether the provided API credentials are for an enterprise account. Defaults to false.
    public init(individualPrivateKeyID: String? = nil, privateKeyURL: URL, expirationDuration: TimeInterval = 60 * 20, isEnterprise: Bool = false) throws {
        self.issuerID = nil
...
public final class APIProvider {
    ...
    public func request(_ request: Request<Void>, completion: @escaping RequestCompletionHandler<Void>) {
        guard let request = try? requestsAuthenticator.adapt(request.asURLRequest(encoder: encoder, isEnterprise: configuration.isEnterprise)) else {
...
extension APIProvider {
    ...
    private func pagedDocumentLinks<T>(_ object: T) -> PagedDocumentLinksInterface? {
        let mirror = Mirror(reflecting: object)
        return mirror.children
                        .first(where: {
                            $0.value is PagedDocumentLinksInterface
                        })
                        .flatMap {
                            $0.value as? PagedDocumentLinksInterface
                        }
    }
