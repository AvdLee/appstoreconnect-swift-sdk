...
extension Request {
    ...
    private func baseURL(isEnterprise: Bool) -> URL {
        if isEnterprise {
    ...
    private func makeURL(path: String, query: [(String, String?)]?, isEnterprise: Bool) throws -> URL {
        let url = baseURL(isEnterprise: isEnterprise).appendingPathComponent(path)
    ...
    public func asURLRequest(encoder: JSONEncoder, isEnterprise: Bool) throws -> URLRequest {
        let url = try makeURL(path: path, query: query, isEnterprise: isEnterprise)
