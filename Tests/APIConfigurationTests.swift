...
final class APIConfigurationTests: XCTestCase {
    ...
    func testValidPrivateKey() throws {
        XCTAssertNoThrow(try APIConfiguration(issuerID: issuerID, privateKeyID: privateKeyID, privateKey: privateKey))
    }

    func testNonBase64PrivateKey() throws {
        let nonBase64Key = "&^&%^$%$%"
        XCTAssertThrowsError(try APIConfiguration(issuerID: issuerID, privateKeyID: privateKeyID, privateKey: nonBase64Key)) { (error) in
    ...
    func testInvalidPrivateKey() throws {
        ...
        XCTAssertThrowsError(try APIConfiguration(issuerID: issuerID, privateKeyID: privateKeyID, privateKey: invalidKey)) { (error) in
